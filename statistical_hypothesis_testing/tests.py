...
def getDataForRegion(regionLetter):
...
def performFTest(data1, data2, alpha, alternative):
    statistics = np.var(data1, ddof=1) / np.var(data2, ddof=1)
    df1 = len(data1) - 1
    df2 = len(data2) - 1
    if alternative == "≠":
        print(f"( - ∞, {round(f.ppf(alpha / 2, df1, df2), 3)} > < {round(f.ppf(1 - alpha / 2, df1, df2), 3)} , ∞)")
    elif alternative == "<":
        print(f"( - ∞, {round(f.ppf(alpha, df1, df2), 3)} >")
    elif alternative == ">":
...
def performTTest(data1, data2, equalVar, alpha, alternative):
